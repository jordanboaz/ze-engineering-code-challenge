...
const ProductControl = (props: Props) => {
  ...
  const [units, setUnits] = useState([1, ...(props.units?.length ? props.units : [])]);
...
<ControlsContainer key={key}>
      ...
      <ControlValue value={String(value)} onChangeText={onChangeValue} keyboardType="numeric" />
  ...
  return <Container>
    <UnitsControlContainer>
      {units.map((eachUnit: number, index: number) => {
        ...
        return <Unit key={eachUnit} onPress={() => selectUnit(eachUnit, index)}>
